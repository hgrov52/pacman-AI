...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # edge cases, when a ghost is found, set B[position] to 1 and others to 0
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0: 
                # see observation update equation on lecture slides
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos in newPosDist:
                # see lecture slides on "Passage of Time"
                beliefs[newPos] += newPosDist[newPos] * self.beliefs[oldPos]
        beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # call initializeUniformly() on gameState when weights is 0
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # exactly the same as the above elapseTime function
        # beliefs = util.Counter()
        # for oldPos in self.legalPositions:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        #     for newPos in newPosDist:
        ...
        for p in self.legalPositions:
            possible = self.setGhostPosition(gameState, p)
            distribution = self.getPositionDistribution(possible)
            for position in distribution:
                new_beliefs[position]+=distribution[position]*self.beliefs[p]
