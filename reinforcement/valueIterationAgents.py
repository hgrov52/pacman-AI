...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            for state in states:
                ...
                if(mdp.isTerminal(state)==False):
                    ...
                    for action in actions:
                        ...
                        action_val = max(action_val, q_val)
                    self.values[state] =action_val
            self.prev_values = self.values.copy()
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, probs in self.mdp.getTransitionStatesAndProbs(state, action):
            q_val += probs *(self.discount*self.prev_values[nextState] +self.mdp.getReward(state, action, nextState))
        return q_val
